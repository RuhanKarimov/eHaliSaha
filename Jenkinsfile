...
'''
  ...
  post {
      ...
      if (isUnix()) {
                        withEnv(envs) { sh "./mvnw ${env.MVN_ARGS} -P e2e -Dtest=Scenario02OwnerCreateFacilityTestE2E test" }
      } else {
                        withEnv(envs) { bat ".\\mvnw.cmd %MVN_ARGS% -P e2e -Dtest=Scenario02OwnerCreateFacilityTestE2E test" }
      }
    }
  }
}
