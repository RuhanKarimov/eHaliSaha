...
def test_member_cannot_request_membership_when_already_active():
    """Üyelik ACTIVE iken tekrar istek atmaya çalışınca backend hata dönmeli, UI bunu göstermeli."""
    wait_for_app()
    ...
    try:
        ...
        txt = out.text.lower()
        assert ("already" in txt) or ("active" in txt) or ("istek" in txt) or ("hata" in txt), f"Beklenen hata mesajı yok: {out.text}"
    finally:
        driver.quit()
